...
class DataProcessor:
    ...
    def _convert_to_features(self, datum):
        ...
        src_tokenized = [self.lang_id_token] + self.tokenizer.tokenize(src_txt)[:self.max_source_length-1]

        # SOURCE
        tgt_txt = tgt
        tgt_tokenized = [self.lang_id_token] + self.tokenizer.tokenize(tgt_txt)[:self.max_target_length-2] + [self.tgt_eos]
...
def load_mydataset(path):
    ...
    for idx, row in pd.read_csv(path).iterrows():
        src = [row['Kalimat-1'], row['Kalimat-2'], row['Kalimat-3'], row['Kalimat-4']]
        tgt = row['Correct Ending']
        data.append({
            'src': src,
            'tgt': tgt
            })
...
def main():
    ...
    train_dataset = load_mydataset(args.dataset_path+'/train.csv')
    dev_dataset = load_mydataset(args.dataset_path+'/dev.csv')
    test_dataset = load_mydataset(args.dataset_path+'/test.csv')
